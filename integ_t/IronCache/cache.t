...
subtest 'Put and query items' => sub {
	...
	diag(q{Put item into cache '} . $unique_cache_name_01 . q{', item key '} . $iron_cache_o1_item_01_key . q{'.});
	...
	diag(q{Put item into cache '} . $unique_cache_name_01 . q{', item key '} . $iron_cache_o1_item_02_key . q{'.});
	...
	diag(q{Item in cache '} . $unique_cache_name_01 . q{', item key '} . $iron_cache_o1_item_02_key . q{', value } . $iron_cache_o1_item_02_value . q{ .});
	...
	like($EVAL_ERROR, '/IronHTTPCallException: status_code=404 response_message=Key not found/', 'Correct IronHTTPCallException');
...
subtest 'Clean up.' => sub {
	...
	diag(q{got info on cache '} . $unique_cache_name_01 . q{', size is } . $queried_iron_cache_info_02->{'size'} . q{.});
	...
	throws_ok {
		my $is_deleted = $iron_cache_client->get_cache(
			...
			'Throws IronHTTPCallException when cache not found with given name.';
	like($EVAL_ERROR, '/IronHTTPCallException: status_code=404 response_message=Cache not found/', 'Correct IronHTTPCallException');
	diag(q{Tried to get cache '} . $unique_cache_name_01 . q{' which does not exist. Threw ok.});
