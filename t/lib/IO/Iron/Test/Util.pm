...
sub create_unique_queue_name {
	...
	my $unique_queue_name =
	  'TESTQUEUE_' . (substr $ug->to_string($uuid1), 1, $QUEUE_NAME_LENGTH);
...
sub create_unique_cache_name {
	...
	my $unique_cache_name =
	  'TESTCACHE_' . (substr $ug->to_string($uuid1), 1, $CACHE_NAME_LENGTH);
...
sub create_unique_code_package_name {
	...
	my $unique_code_package_name =
	  'TESTWORKER_' . (substr $ug->to_string($uuid1), 1, $CODE_PACKAGE_NAME_LENGTH);
